                  ...
                  <span @click="cropPicture(false)">
                    <slot name="confirm">
                      <button
                        ...
                        :disabled="!drawImg.img">确定</button>
                    </slot>
                  </span>
                </div>
              </div>
...
export default {
  ...
  methods: {
    ...
    importImgToCanv: function (img) {
      ...
      if (true === this.saveCutPosition) {
        drawImg.x = this.cachePosition.x
        drawImg.y = this.cachePosition.y
        if (this.cachePosition.width > 0 && this.cachePosition.height > 0) {
          drawImg.width = this.cachePosition.width
          drawImg.height = this.cachePosition.height
        }
      } else {
    ...
    dropImgOff: function () {
      ...
      console.log('aaaaa', this.drawImg)
      this.onPrintImgTimmer = setTimeout(() => {
        this.cropPicture(true)
      }, 100)
    },
    dropImgMove: function (e) {
      ...
      if (this.dropImg.active && this.drawImg.img) {
        ...
        if (true === this.saveCutPosition) {
          this.cachePosition.x = drawImg.x
          this.cachePosition.y = drawImg.y
        }
        _this.$set(_this, 'drawImg', drawImg)
        _this.printImg()
        e.stopPropagation()
    ...
    scaleImgWheel: function (e) {
      ...
      if (_this.drawImg.img && this.scaleAble === true) {
        ...
        if (true === this.saveCutPosition) {
          this.cachePosition.width = _this.drawImg.width
          this.cachePosition.height = _this.drawImg.height
        }
        _this.printImg()
        if (this.onPrintImgTimmer) {
          clearTimeout(this.onPrintImgTimmer)
